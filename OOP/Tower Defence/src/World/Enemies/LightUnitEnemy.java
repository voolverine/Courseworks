...
public class LightUnitEnemy extends Enemy implements IHealthDrawable {
    ...
    public void setPosition(int new_x, int new_y) {
        ...
        if (dx > 0 && dy > 0) {
            if (cur >= right_down1ID && cur <= right_down3ID) {
                if (cur == right_down1ID) {
                    ImageID = right_down2ID;
                } else
                if (cur == right_down2ID) {
                    ImageID = right_down3ID;
                } else {
                    ImageID = right_down1ID;
                }
            } else {
                ImageID = right_down1ID;
            }
        ...
        if (dx > 0 && dy < 0) {
            if (cur >= right_up1ID && cur <= right_up2ID) {
                if (cur == right_up1ID) {
                    ImageID = right_up2ID;
                } else {
                    ImageID = right_up1ID;
        ...
        if (dx < 0 && dy > 0) {
            if (cur >= left_down1ID && cur <= left_down3ID) {
                if (cur == left_down1ID) {
                    ImageID = left_down2ID;
                } else
                if (cur == left_down2ID) {
                    ImageID = left_down3ID;
                } else {
                    ImageID = left_down1ID;
                }
            } else {
                ImageID = left_down1ID;
            }
        ...
        if (dx < 0 && dy < 0) {
            if (cur >= left_up1ID && cur <= left_up2ID) {
                if (cur == left_up1ID) {
                    ImageID = left_up2ID;
                } else {
                    ImageID = left_up1ID;
